...
// we should delete file need to free inode
int free_inode(int index) 
{
		inodeMap[index / 8] ^= 1 << (index % 8);
		if(get_bit(inodeMap, index) == 1) 
		{
				// if the location not update to 0, roll back
				toggle_bit(inodeMap, index);
				return -1;
		}
		superBlock.freeInodeCount++;
		return 0;
...
// we shold delete file need to free block
int free_block(int index) {

		blockMap[index / 8] ^= 1 << (index % 8);
		if(get_bit(blockMap, index) == 1) 
		{
				// if the location not update to 0, roll back
				toggle_bit(blockMap, index);
				return -1;
		}
		superBlock.freeBlockCount++;
		return 0;}
